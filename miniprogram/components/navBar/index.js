Component({
  ...
  methods: {
    ...
    // 根据页面的分类进行导航栏样式的设置
    getPageLevel() {
      // 首页导航栏左上角显示home、中部title -> -0
      // 嵌套页面左上角显示返回上一页 + 回到首页按钮 -> 1
      // 非常规首页左上角显示回到首页按钮 -> 2
      const pages = getCurrentPages()
      let current_page = pages[pages.length - 1].route
      const NORMAL_ENTRY = getApp().globalData.NORMAL_ENTRY
      console.log(current_page)
      let defaultLevel = 0
      if (pages.length === 1 && current_page === NORMAL_ENTRY) {
        defaultLevel = 0
      } else if (pages.length === 1 && current_page !== NORMAL_ENTRY) {
        defaultLevel = 2
    ...
    setNavLevel() {
      if (!this.properties.control) {
        this.setData({
          showBack: this.properties.back,
          showHome: this.properties.home
        })
      } else {
        let level = this.getPageLevel()
        let showBack = false
        let showHome = false
        if (level === 0) {
          showHome = true
        } else if (level === 1) {
          showHome = showBack = true
        } else {
          showHome = true
        }
        this.setData({
